...
export type SiteMessageCategory='Material fehlt'|'Verbrauchsmaterial fehlt'|'Schrauben fehlen'|'Silikon fehlt'|'Dichtband fehlt'|'Werkzeug fehlt'|'Werkzeug defekt'|'Werkzeug verloren'|'Fahrzeugproblem'|'TÜV fällig'|'Service fällig'|'Schaden am Fahrzeug'|'Material beschädigt'|'Falsches Material geliefert'|'Nachbestellung'|'Mangel'|'Nacharbeit'|'Abnahme'|'Zustandsbilder'|'Sicherheitsproblem'|'Kundenwunsch'|'Sonstiges';
...
export type ToolStatus='Verfügbar'|'Ausgegeben'|'Auf Baustelle'|'Defekt'|'In Reparatur'|'Verloren'|'Ausgemustert';

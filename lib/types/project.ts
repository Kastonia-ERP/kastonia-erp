...
export interface Task { id: string; title: string; due: ISODateString; priority: ProjectPriority; done: boolean; projectId?: string; assignedTo?: string; description?: string; project?: string; customer?: string; responsible?: string; status?: 'Offen'|'In Bearbeitung'|'Wartet'|'Erledigt'; category?: 'Angebot'|'Einkauf'|'Montage'|'Büro'|'Buchhaltung'|'Nacharbeit'|'Kunde'|'Material'|'Sonstiges'; attachments?: {id:string; name:string; url?:string}[]; comments?: {id:string; createdAt:ISODateString; author:string; text:string}[]; }
...
export const APP_VERSION_LABEL='v1.7 · Operations Center';

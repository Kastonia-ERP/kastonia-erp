...
export default function Nav(){const p=usePathname();return <aside className="nav"><div className="brand"><span>K</span><div><b>KASTONIA ERP</b><small>v1.9.3 · Personnel Availability & Annual Workforce Planning</small></div></div><nav>{items.map(([n,h])=><Link className={p===h?'active':''} href={h} key={h}>{n}</Link>)}</nav><div className="user"><b>Gökhan Karayel</b><small>Administrator · Cloud-ready · Supabase vorbereitet</small></div></aside>}

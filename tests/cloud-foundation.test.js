...
test('version and env example expose cloud foundation',()=>{assert.match(read('package.json'),/"version": "1\.8\.0"/);const env=read('.env.example');for(const key of ['NEXT_PUBLIC_SUPABASE_URL','NEXT_PUBLIC_SUPABASE_ANON_KEY','SUPABASE_SERVICE_ROLE_KEY','NEXT_PUBLIC_APP_URL','NEXT_PUBLIC_STORAGE_BUCKET','NEXT_PUBLIC_APP_ENV'])assert.match(env,new RegExp(key));});
